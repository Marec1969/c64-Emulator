...
void ADC_A(uint8_t value) {
    if (cpu.SR & FLAG_DECIMAL) { // Dezimalmodus (BCD)
        ...
        uint8_t low_nibble_A = cpu.A & 0x0F;
        uint8_t high_nibble_A = cpu.A >> 4;
        uint8_t low_nibble_value = value & 0x0F;
        uint8_t high_nibble_value = value >> 4;

        // Addiere die unteren Nibbles mit Carry
        uint8_t low_result = low_nibble_A + low_nibble_value + (cpu.SR & FLAG_CARRY ? 1 : 0);
        if (low_result > 9) {
            low_result -= 10; // Korrektur bei BCD
            high_nibble_A++;  // Übertrag zu den höheren Nibbles
        }

        // Addiere die oberen Nibbles
        uint8_t high_result = high_nibble_A + high_nibble_value;
        if (high_result > 9) {
            high_result -= 10; // Korrektur bei BCD
            set_flag(FLAG_CARRY, 1); // Setze Carry, falls Übertrag auftritt
        } else {
            set_flag(FLAG_CARRY, 0);
        }

        // Setze das Ergebnis zusammen
        cpu.A = (high_result << 4) | (low_result & 0x0F);
...
void SBC_A(uint8_t value) {
    if (cpu.SR & FLAG_DECIMAL) { // Dezimalmodus (BCD)
        ...
        uint8_t low_result = low_nibble_A - low_nibble_value - (cpu.SR & FLAG_CARRY ? 0 : 1);
        if (low_result & 0x10) { // Wenn das Ergebnis negativ ist (Unterlauf)
            low_result += 10;   // Korrigiere für BCD
            high_nibble_A--;    // Borrow von den höheren Nibbles
        }

        // Subtrahiere die oberen Nibbles
        uint8_t high_result = high_nibble_A - high_nibble_value;
        if (high_result & 0x10) { // Wenn das Ergebnis negativ ist (Unterlauf)
            high_result += 10;   // Korrektur bei BCD
            set_flag(FLAG_CARRY, 0); // Setze Carry nicht
        } else {
            set_flag(FLAG_CARRY, 1);  // Setze Carry
        }
        ...
        // Setze die entsprechenden Flags
        set_flag(FLAG_CARRY, cpu.A >= (value + (cpu.SR & FLAG_CARRY ? 0 : 1))); // Carry-Flag, wenn kein Unterlauf auftritt
        set_flag(FLAG_ZERO, (result & 0xFF) == 0);             // Zero-Flag, wenn das Ergebnis null ist
