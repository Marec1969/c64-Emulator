...
void OPCODE_30(void) {
    ...
    if (cpu.SR & FLAG_NEGATIVE) {
        cpu.PC += (int8_t)read_memory(cpu.PC); // Zweig ausführen, wenn das Negative-Flag gesetzt ist
         cpu.PC++;
   } else {
      cpu.PC++;
    }
...
void OPCODE_36(void) {
    ...
    write_memory(addr, result);                   // Schreibe den neuen Wert zurück in den Speicher
    cpu.PC++;
}
...
void OPCODE_38(void) {
    ...
    set_flag(FLAG_CARRY, 1);  // Setze das Carry-Flag
    cpu.PC++;
}
...
void OPCODE_3C(void) {
    ...
    read_memory(addr);                           // Lese nur den Speicher, mache aber nichts
    cpu.PC++;
}
